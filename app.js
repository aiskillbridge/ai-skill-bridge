...
async function signInWithGoogle() {
  if (/FBAN|FBAV|Instagram|Line|MicroMessenger|Messenger/i.test(navigator.userAgent)) {
    alert(state.lang === "zh"
      ? "請使用 Safari 或 Chrome 開啟網站後再登入 Google。LINE/IG/FB 內建瀏覽器會被 Google 擋住。"
      : "Please open this website in Safari or Chrome before Google login. In-app browsers may be blocked by Google.");
    return;
  }

  if (!supabaseClient) {
    alert(state.lang === "zh"
      ? "尚未設定 Supabase Publishable Key。"
...
function nav() {
  const items = [
    ["home", "nav.home"],
    ["progress", state.lang === "zh" ? "我的進度" : "My Progress"],
    ["courses", "nav.courses"],
    ["free", "nav.free"],
    ["premium", "nav.premium"],
    ["tools", "nav.tools"],
  ...
  const authHtml = state.user
    ? `<button class="lang" onclick="setRoute('progress')" title="${state.user.email}">${state.profile?.display_name || state.user.email.split("@")[0]}</button><button class="lang" onclick="signOut()">${state.lang === "zh" ? "登出" : "Logout"}</button>`
    : `<button class="lang" onclick="signInWithGoogle()">${state.lang === "zh" ? "Google 登入" : "Google Login"}</button>`;

  return `<header class="topbar"><div class="wrap nav"><button class="brand" onclick="setRoute('home')"><span class="logo">AI</span><span>Skill <span style="color:var(--blue)">Bridge</span></span></button><nav class="navlinks">${items.map(([route, label]) => `<button class="${state.route === route ? "active" : ""}" onclick="setRoute('${route}')">${String(label).startsWith("nav.") ? L(label) : label}</button>`).join("")}${authHtml}<button class="lang" onclick="toggleLang()">${state.lang === "zh" ? "EN" : "中文"}</button></nav></div></header>`;
}
...
function home() {
  return shell(`<main><section class="hero"><div class="wrap hero-grid"><div><p class="eyebrow">${L("home.eyebrow")}</p><h1>${L("home.title")}</h1><p class="lead">${L("home.lead")}</p>${state.user ? `<div class="card" style="margin:20px 0"><b>${state.lang === "zh" ? "已登入：" : "Signed in as:"}</b> ${state.user.email}</div>` : `<div class="card" style="margin:20px 0"><b>${state.lang === "zh" ? "會員功能：" : "Member Feature:"}</b> ${state.lang === "zh" ? "登入後可同步學習進度、未來解鎖付費課程。" : "Login will later sync progress and unlock premium courses."}</div>`}<div class="btnrow"><button class="btn primary" onclick="setRoute('free')">${L("home.start")}</button><button class="btn secondary" onclick="setRoute('tools')">${L("home.tools")}</button><button class="btn secondary" onclick="setRoute('premium')">${L("home.premium")}</button></div></div><aside class="hero-card"><h2>${L("home.progress")}</h2><div class="progressbar"><div class="progressfill" style="width:${progressPercent()}%"></div></div><p>${progressPercent()}% completed</p><div class="grid two"><div class="card"><b>${LESSONS.length}</b><br>${L("home.freeLessons")}</div><div class="card"><b>${TOOLS.length}</b><br>${L("home.aiTools")}</div><div class="card"><b>${PREMIUM.length}</b><br>${L("home.premiumTracks")}</div><div class="card"><b>2</b><br>${L("home.languages")}</div></div></aside></div></section><section class="section"><div class="wrap"><h2>${L("home.missionTitle")}</h2><p class="lead">${L("home.missionText")}</p><div class="grid three"><article class="card"><span class="tag free">Free</span><h3>${L("home.freeTitle")}</h3><p>${L("home.freeText")}</p></article><article class="card"><span class="tag premiumtag">Premium</span><h3>${L("home.premiumTitle")}</h3><p>${L("home.premiumText")}</p></article><article class="card"><span class="tag communitytag">Community</span><h3>${L("home.communityTitle")}</h3><p>${L("home.communityText")}</p></article></div></div></section></main>`);
}

function courses() {
  return shell(`<main class="page"><div class="wrap"><h1>${L("courses.title")}</h1><p class="lead">${L("courses.lead")}</p><div class="grid two"><article class="pricing"><span class="tag free">Free</span><h2>${L("courses.freePath")}</h2><p>${L("courses.freePathDesc")}</p><p class="price">NT$0</p><button class="btn primary" onclick="setRoute('free')">${L("courses.enter")}</button></article><article class="pricing"><span class="tag premiumtag">Premium</span><h2>${L("courses.premiumPath")}</h2><p>${L("courses.premiumPathDesc")}</p><p class="price">NT$499+</p><button class="btn secondary" onclick="setRoute('premium')">${L("courses.view")}</button></article></div><section class="panel" style="margin-top:24px"><h2>${L("courses.levelTitle")}</h2><div class="grid four"><article class="card"><h3>Level 0</h3><p><b>${L("courses.level0")}</b></p><p>${L("courses.level0Goal")}</p></article><article class="card"><h3>Level 1</h3><p><b>${L("courses.level1")}</b></p><p>${L("courses.level1Goal")}</p></article><article class="card"><h3>Level 2</h3><p><b>${L("courses.level2")}</b></p><p>${L("courses.level2Goal")}</p></article><article class="card"><h3>Advanced</h3><p><b>${L("courses.advanced")}</b></p><p>${L("courses.advancedGoal")}</p></article></div></section></div></main>`);
}
...
function tools() {
  ...
  return shell(`<main class="page"><div class="wrap"><h1>${L("tools.title")}</h1><p class="lead">${L("tools.lead")}</p><section class="panel"><h2>${L("tools.taskTitle")}</h2><div class="grid three">${scenarios.map(([title, names]) => `<article class="card"><h3>${title}</h3><p>${names.map(name => `<span class="tag">${name}</span>`).join(" ")}</p></article>`).join("")}</div></section><div class="grid three">${TOOLS.map(tool => `<article class="card"><div class="tool-logo">${tool.name[0]}</div><h3>${tool.name}</h3><p>${state.lang === "zh" ? tool.zh : tool.en}</p><a class="btn primary" href="${tool.url}" target="_blank">${L("tools.open")}</a></article>`).join("")}</div></div></main>`);
}
...
function community() {
  return shell(`<main class="page"><div class="wrap"><h1>${L("community.title")}</h1><p class="lead">${L("community.lead")}</p><div class="slide"><div><p class="eyebrow" style="color:#93c5fd">${L("community.boardTitle")}</p><h3>${L("community.boardQuestion")}</h3></div><div class="sticky-note">${L("community.note")}</div></div><div class="grid two"><article class="post"><span class="tag communitytag">Prompt Help</span><h3>How do I ask AI to help with research?</h3><p>Add constraints: do not write final answer, ask questions first, verify sources.</p><button class="btn secondary" onclick="toast('${L("community.demo")}')">${L("community.reply")}</button></article></div></div></main>`);
}

function thailand() {
  return shell(`<main class="page"><div class="wrap"><h1>${L("thailand.title")}</h1><p class="lead">${L("thailand.lead")}</p><section class="panel"><h2>${L("thailand.journal")}</h2><div class="grid four"><article class="card"><h3>${L("thailand.daily")}</h3><p>${L("thailand.dailyText")}</p></article><article class="card"><h3>${L("thailand.field")}</h3><p>${L("thailand.fieldText")}</p></article><article class="card"><h3>${L("thailand.project")}</h3><p>${L("thailand.projectText")}</p></article><article class="card"><h3>${L("thailand.action")}</h3><p>${L("thailand.actionText")}</p></article></div></section></div></main>`);
}
...
function progressPage() {
  if (!state.user) {
    return shell(`<main class="page"><div class="wrap">
      <h1>${state.lang === "zh" ? "我的進度" : "My Progress"}</h1>
      <p class="lead">${state.lang === "zh" ? "請先登入 Google 帳號，才能查看個人學習進度。" : "Please login with Google to view your learning progress."}</p>
      <button class="btn primary" onclick="signInWithGoogle()">${state.lang === "zh" ? "Google 登入" : "Google Login"}</button>
    </div></main>`);
  }

  return shell(`<main class="page"><div class="wrap">
    <h1>${state.lang === "zh" ? "我的學習進度" : "My Learning Progress"}</h1>
    <p class="lead">${state.lang === "zh" ? "這裡會顯示你的會員資料、學習進度和目前等級。" : "Your member profile, course progress, and current level."}</p>

    <div class="grid three">
      <article class="card">
        <h3>${state.lang === "zh" ? "會員資料" : "Profile"}</h3>
        <p><b>${state.lang === "zh" ? "名稱" : "Name"}：</b>${state.profile?.display_name || state.user.email}</p>
        <p><b>Email：</b>${state.profile?.email || state.user.email}</p>
        <p><b>${state.lang === "zh" ? "方案" : "Plan"}：</b>${state.profile?.plan || "free"}</p>
      </article>

      <article class="card">
        <h3>${state.lang === "zh" ? "學習進度" : "Progress"}</h3>
        <div class="progressbar"><div class="progressfill" style="width:${progressPercent()}%"></div></div>
        <p><b>${completedCount()} / ${LESSONS.length}</b> ${state.lang === "zh" ? "課已完成" : "lessons completed"}</p>
        <p>${progressPercent()}% completed</p>
      </article>

      <article class="card">
        <h3>${state.lang === "zh" ? "目前等級" : "Current Level"}</h3>
        <p class="price">${currentLevel()}</p>
        <p>${state.lang === "zh" ? "完成免費入門課程後，即可進入 Level 1 應用課程。" : "Complete all free lessons to become Level 1 ready."}</p>
      </article>
    </div>

    <section class="panel" style="margin-top:24px">
      <h2>${state.lang === "zh" ? "課程完成狀態" : "Lesson Status"}</h2>
      <div class="grid three">
        ${LESSONS.map(lesson => `
          <article class="card">
            <span class="tag ${state.progress[lesson.id] ? "free" : ""}">
              ${state.progress[lesson.id] ? "✓ Completed" : (state.lang === "zh" ? "尚未完成" : "Not yet")}
            </span>
            <h3>${state.lang === "zh" ? lesson.zhTitle : lesson.enTitle}</h3>
            <p>${lesson.level}</p>
            <button class="btn secondary" onclick="state.activeLesson='${lesson.id}';setRoute('free')">${state.lang === "zh" ? "前往課程" : "Open lesson"}</button>
          </article>
        `).join("")}
      </div>
    </section>
