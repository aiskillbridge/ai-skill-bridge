...
async function loadProgressFromSupabase() {
  ...
  if (error) {
    ...
    toast(state.lang === "zh" ? "讀取學習進度失敗" : "Failed to load progress");
    return;
  }
...
async function completeLesson(lessonId) {
  ...
  if (!state.user || !supabaseClient) {
    toast(state.lang === "zh" ? "已儲存在本機。登入後可同步進度。" : "Saved locally. Login to sync progress.");
    return;
  }
  ...
  if (selectError) {
    ...
    toast(state.lang === "zh" ? "同步前檢查失敗" : "Sync check failed");
    return;
  }
  ...
  if (existing?.id) {
    ...
    if (error) {
      ...
      toast(state.lang === "zh" ? "進度更新失敗" : "Progress update failed");
      return;
    }
  } else {
    const { error } = await supabaseClient
    ...
    if (error) {
      ...
      toast(state.lang === "zh" ? "進度新增失敗" : "Progress insert failed");
      return;
    }
  }

  toast(state.lang === "zh" ? "學習進度已同步" : "Progress synced");
}
            ...
            ${
              state.user
                ...
                : `<div class="card" style="margin:20px 0">
                    ...
                    ${state.lang === "zh" ? "登入後可同步學習進度、未來解鎖付費課程。" : "Login will sync progress and later unlock premium courses."}
                  </div>`
            }
